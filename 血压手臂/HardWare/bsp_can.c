...
static void CAN_Mode_Config(void)
{
  ...
  RCC_APB1PeriphClockCmd(CAN_CLK, ENABLE);
	...
	CAN_InitStructure.CAN_BS1=CAN_BS1_7tq;		   //BTR-TS1 时间段1 占用了5个时间单元
...
static void CAN_Filter_Config(void)
{
	...
	CAN_FilterInitStructure.CAN_FilterMode=CAN_FilterMode_IdList;	//工作在掩码模式
	CAN_FilterInitStructure.CAN_FilterScale=CAN_FilterScale_16bit;	//筛选器位宽为单个32位。
	/* 使能筛选器，按照标志的内容进行比对筛选，扩展ID不是如下的就抛弃掉，是的话，会存入FIFO0。 */
	CAN_FilterInitStructure.CAN_FilterIdHigh = 0x27;
	CAN_FilterInitStructure.CAN_FilterIdLow = 0x27;
	if(Is_LeftOrRight() == Blood_LEFT)
	{
		CAN_FilterInitStructure.CAN_FilterIdHigh= ((u16)0x11<<5);		//要筛选的ID高位 
		rt_kprintf("CAN左");		
	}
	if(Is_LeftOrRight() == Blood_RIGHT)
	{
		CAN_FilterInitStructure.CAN_FilterIdLow= ((u16)0x21<<5); //要筛选的ID低位 
		rt_kprintf("CAN右");
	}
	CAN_FilterInitStructure.CAN_FilterMaskIdHigh= 0x80<<5;			//筛选器高16位每位必须匹配
	CAN_FilterInitStructure.CAN_FilterMaskIdLow= 0x16<<5;			//筛选器低16位每位必须匹配
 ...
 * 调用  ：外部调用
 */	 
void CAN_SetMsg(uint8_t addr,CanTxMsg *TxMessage)
{	  
	TxMessage->StdId=addr;						 			
//  TxMessage->ExtId=0x13;					 //使用的扩展ID
